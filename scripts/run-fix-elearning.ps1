...
$instanceId = aws ec2 describe-instances --profile account-483382415631 --filters "Name=tag:aws:cloudformation:stack-name,Values=MoodleCdkStack" "Name=instance-state-name,Values=running" --query "Reservations[0].Instances[0].InstanceId" --output text
