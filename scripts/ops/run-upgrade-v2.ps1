...
echo "=== Clear any upgrade lock ==="
$DB -e "DELETE FROM mdl_config WHERE name='upgraderunning';" 2>&1

echo "=== Run upgrade.php ==="
timeout 180 php /app/moodle/admin/cli/upgrade.php --non-interactive 2>&1
echo "Exit: $?"

echo "=== DB state after upgrade ==="
$DB -e "SELECT name,value FROM mdl_config WHERE name IN ('version','upgraderunning','allversionshash');" 2>&1

echo "=== Test upgrade status ==="
timeout 30 php -r "
...
$paramsFile = Join-Path $env:TEMP 'upgrade-v2-params.json'
