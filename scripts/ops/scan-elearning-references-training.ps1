...
$cmdId = aws ssm send-command --region $region --document-name AWS-RunShellScript --comment 'Scan for elearning domain references' --instance-ids $idsArr --parameters file://scripts/ssm-scan-elearning-references.json --query 'Command.CommandId' --output text
...
Do {
  ...
  foreach($id in $idsArr) {
    if ($status[$id] -eq 'Success' -or $status[$id] -eq 'Failed') { continue }
    try {
