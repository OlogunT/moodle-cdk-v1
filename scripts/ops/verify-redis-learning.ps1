...
$cmdId = aws ssm send-command --region $region --document-name AWS-RunShellScript --comment 'Verify Redis config on Learning Moodle' --instance-ids $idsArr --parameters file://scripts/ssm-verify-redis-learning.json --query 'Command.CommandId' --output text
