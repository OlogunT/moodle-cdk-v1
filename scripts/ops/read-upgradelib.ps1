...
$p = Join-Path $env:TEMP 'read-ul.json'

$bash = @'
pkill -9 -f "find_mm\|upgrade.php\|cron.php" 2>/dev/null || true
echo "=== Search ==="
grep -rn "function moodle_needs_upgrading" /app/moodle/lib/*.php 2>/dev/null
echo "=== grep2 ==="
grep -rn "moodle_needs_upgrading" /app/moodle/lib/setuplib.php 2>/dev/null | head -5
echo "=== DONE ==="
'@
